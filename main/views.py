...
def index(request):
    ...
    if query and selected_categories:        
        articles = articles.filter(title__iregex=query, category_id__in=selected_categories)
...
def article_detail(request, article_id):
    ...
    if request.method == "POST":
        ...
        if username and comment_text:
            ...
            return redirect(f"{request.path}?{query_params}")  # Обновляем страницу с сохранением параметров
